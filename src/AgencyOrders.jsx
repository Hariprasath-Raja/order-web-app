    ...
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            onClick={fetchOrders}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold rounded-xl transition active:scale-95"
          >
            <Icons.Refresh />
            <span>Refresh</span>
          </button>
                ...
                filteredOrders.map((ord) => (
                  <tr key={ord.OrderId} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="py-3.5 px-4">
                      <span className="font-bold text-slate-800 block">{ord.ShopNameSnapshot}</span>
                      <span className="text-[11px] text-slate-400 flex items-center gap-1.5 mt-0.5">
                        <span>{ord.OwnerNameSnapshot}</span>
                        <span>•</span>
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl text-xs transition active:scale-95 shadow-xs"
                      >
                        <span>Items</span>
                        <Icons.ArrowRight />
                      </button>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex justify-end">
          <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col p-6 animate-in slide-in-from-right">
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-slate-100 py-3">
              {orderItems.map((it) => (
                <div key={it.OrderItemId} className="py-3 flex justify-between items-center text-xs">
                  <div>
            ...
            <div className="pt-4 border-t border-slate-200 space-y-2 text-xs">
              ...
              <div className="pt-3">
                ...
                <select
                  ...
                  className="w-full text-xs font-bold bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 focus:outline-none"
                >
                  {statusList.map((st) => (
                    <option key={st.StatusCode} value={st.StatusCode}>
                      {st.StatusName}
